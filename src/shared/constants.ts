export const IPC_CHANNELS = {
  TASK_CREATE: 'task:create',
  TASK_UPDATE: 'task:update',
  TASK_START: 'task:start',
  TASK_PAUSE: 'task:pause',
  TASK_STOP: 'task:stop',
  TASK_DELETE: 'task:delete',
  TASK_GET: 'task:get',
  TASK_LIST: 'task:list',
  TASK_GET_STEPS: 'task:get-steps',
  TASK_CREATE_STEP: 'task:create-step',
  TASK_UPDATE_STEP: 'task:update-step',
  TASK_DELETE_STEP: 'task:delete-step',
  TASK_STATUS_CHANGED: 'task:status-changed',
  TASK_STEP_RESULT: 'task:step-result',
  TASK_LOG: 'task:log',
  TASK_GROUP_CREATE: 'task-group:create',
  TASK_GROUP_UPDATE: 'task-group:update',
  TASK_GROUP_START: 'task-group:start',
  TASK_GROUP_STOP: 'task-group:stop',
  TASK_GROUP_DELETE: 'task-group:delete',
  TASK_GROUP_LIST: 'task-group:list',
  TASK_GROUP_GET: 'task-group:get',
  TASK_GROUP_GET_ITEMS: 'task-group:get-items',
  TASK_GROUP_ADD_ITEM: 'task-group:add-item',
  TASK_GROUP_REMOVE_ITEM: 'task-group:remove-item',
  TASK_GROUP_STATUS_CHANGED: 'task-group:status-changed',
  TASK_GROUP_LOG: 'task-group:log',
  NETWORK_REQUEST: 'network:request',
  NETWORK_START: 'network:start',
  NETWORK_STOP: 'network:stop',
  NETWORK_GET_LOGS: 'network:get-logs',
  NETWORK_EXPORT: 'network:export',
  CAPTURE_SCREENSHOT: 'capture:screenshot',
  BROWSER_CAPTURE_SCREENSHOT: 'browser:capture-screenshot',
  CAPTURE_CLICK: 'capture:click',
  PYTHON_CLICK: 'python:click',
  PYTHON_CLICK_ABORT: 'python:click-abort',
  PYTHON_MOVE: 'python:move',
  CAPTURE_UPDATED: 'capture:updated',
  LOG_ENTRY: 'log:entry',
  LOG_DEBUG_STATE: 'log:debug-state',
  LOG_SET_DEBUG: 'log:set-debug',
  LOG_GET_LOGS: 'log:get-logs',
  LOG_EXPORT: 'log:export',
  LOG_CLEAR_DISPLAY: 'log:clear-display',
  IMPORT_EXPORT_EXPORT: 'import-export:export',
  IMPORT_EXPORT_IMPORT: 'import-export:import',
  BROWSER_LOAD_URL: 'browser:load-url',
  BROWSER_GET_URL: 'browser:get-url',
  BROWSER_SET_BOUNDS: 'browser:set-bounds',
  BROWSER_GO_BACK: 'browser:go-back',
  BROWSER_GO_FORWARD: 'browser:go-forward',
  BROWSER_RELOAD: 'browser:reload',
  BROWSER_LOADING_STATE: 'browser:loading-state',
  TASK_GROUP_UPDATE_LOOP: 'task-group:update-loop',
  TASK_GROUP_UPDATE_ITEM_TARGET: 'task-group:update-item-target',
  TASK_GROUP_REORDER_ITEMS: 'task-group:reorder-items',
  BROWSER_RESIZED: 'browser:resized',
  BROWSER_GET_SIZE: 'browser:get-size',
  BROWSER_GET_POSITION: 'browser:get-position',
  BROWSER_GET_WEBVIEW_POSITION: 'browser:get-webview-position',
  KEY_ESC: 'key:esc',
  KEY_SPACE: 'key:space',
  BROWSER_WINDOW_RESIZED: 'browser:window-resized',
  TASK_CLEAR_COORDINATE_CACHE: 'task:clear-coordinate-cache',
  IMAGE_PICK: 'image:pick',
  IMAGE_NORMALIZE: 'image:normalize',
  STEP_GROUP_LIST: 'step-group:list',
  STEP_GROUP_CREATE: 'step-group:create',
  STEP_GROUP_UPDATE: 'step-group:update',
  STEP_GROUP_DELETE: 'step-group:delete',
} as const;

export type IPCChannel = (typeof IPC_CHANNELS)[keyof typeof IPC_CHANNELS];
